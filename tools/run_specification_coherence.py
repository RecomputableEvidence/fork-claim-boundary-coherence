...
for base in ["specs","canonical","registry","schemas"]:
    for p in (ROOT/base).rglob("*"):
        if p.is_file() and p.suffix.lower() in {".md",".json"}:
            ...
            placeholder_pattern = re.compile(r"(?<![A-Z0-9_-])(TBD|PLACEHOLDER|TODO)(?![A-Z0-9_-])")
            for match in placeholder_pattern.finditer(text):
                placeholder_hits.append({"file": str(path.relative_to(ROOT)), "token": match.group(1), "offset": match.start()})
...
payload={
    ...
    "runner":{"name":"FORK_SPECIFICATION_COHERENCE_RUNNER","version":"0.1.1","digest":runner_digest,"execution_status":"EXECUTED"},
    "claim_boundary":{
        "establishes":[
